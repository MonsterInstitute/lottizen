...
export function generateMetadata({ params }: { params: { game: string } }): Metadata {
  ...
  return {
    title,
    description,
    alternates: { canonical: `/canada/${g.slug}/statistics` },
    openGraph: { title, description, url: absUrl(`/canada/${g.slug}/statistics`) },
  };
}

function Chips({ slug, nums }: { slug: string; nums: number[] }) {
  return (
    <div className="mini-balls">
      {nums.map((n) => (
        <Link key={n} href={`/canada/${slug}/number/${n}`} className="ball sm">
...
export default function StatisticsPage({ params }: { params: { game: string } }) {
  const g = getLiveGame(params.game);
  if (!g) notFound();
    ...
    <>
      <JsonLd
        data={{
          ...
          url: absUrl(`/canada/${g.slug}/statistics`),
      ...
      <div className="page-head">
        <div className="container">
          <div className="breadcrumb">
            ...
            <Link href={`/canada/${g.slug}`}>{g.name}</Link> / <span>Statistics</span>
          </div>
          ...
          <p className="section-lede">
            Computed from {stats.drawCount} draws since{" "}
            {stats.dataSince ? drawDate(stats.dataSince) : "—"}. Frequencies are a record
            of the past — every future draw is still independent and random.
          </p>
          <GameTabs slug={g.slug} active="statistics" />
        </div>
      </div>
        ...
        <div className="container">
          {thin && (
            <div className="notice" style={{ marginBottom: 28 }}>
              ...
              <span>
                Only {stats.drawCount} draw{stats.drawCount === 1 ? "" : "s"} on record for{" "}
                {g.name} so far — a full history source isn&rsquo;t publicly available, so
                these stats deepen as the daily tracker records new draws.
              </span>
            </div>
          )}
          ...
          <div className="chart-card" style={{ marginBottom: 24 }}>
            ...
            <div className="sub">
              How many times each number (1–{stats.max}) has been drawn. Hot numbers in
              deep orange.
            </div>
          ...
          <NumberGrid slug={g.slug} numbers={stats.numbers} />
        </div>
      </section>
    </>
  );
