...
// ============================================================================
// feature_usage — monthly free-run quota for metered Plus features
// (see lib/feature-quota.ts and supabase/migrations/0011_feature_usage.sql).
// ============================================================================
...
export async function getFeatureUsage(
  ...
  const rows = await pg<FeatureUsageRow[]>(
    `feature_usage?subscriber_id=eq.${subscriberId}&feature=eq.${encodeURIComponent(feature)}` +
      `&period=eq.${encodeURIComponent(period)}&select=*`,
  );
  return rows?.[0] ?? null;
}

/** Insert-or-bump this month's counter. on_conflict targets the unique index
 *  (subscriber_id, feature, period) — without an explicit target PostgREST
 *  falls back to the primary key, which a fresh row never collides with, and
 *  the insert 409s instead of merging (the same bug that silently broke
 *  subscription syncing; see upsertSubscriptionByStripeId). */
export async function bumpFeatureUsage(
  subscriberId: string,
  feature: string,
  period: string,
  nextCount: number,
): Promise<void> {
  await pg(`feature_usage?on_conflict=subscriber_id,feature,period`, {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
    body: JSON.stringify([
      {
